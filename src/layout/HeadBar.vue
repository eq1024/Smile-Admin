...
onMounted(async () => {
  await Ent.setEnts()
  Ent.$subscribe((mutation) => {
    if (mutation.storeId === 'ent') {
      if (keepAlivePages?.has(router.currentRoute.value.name as string))
        keepAlivePages.delete(router.currentRoute.value.name as string)
...
<template>
  <header>
    <section>
      <MenuFoldOutlined
        class="icon-sidebar-trigger" :class="[sidebarRelated?.collapsed && 'collapsed']"
        @click="toggleSidebar"
      />
      <BreadCrumb ref="breadCrumbRef" :with-icons="true" />
    </section>
    <ASpace size="middle" style="margin-right: 1rem; font-size: 1rem;">
      <AAutoComplete
        v-model:value="searchValue" style="width: 15rem" :dropdown-match-select-width="250"
        :filter-option="false" :options="searchOptions" @change="searchChange" @select="searchSelect"
      >
        <AInputSearch placeholder="菜单名称/拼音/首字母" allow-clear />
        <template #options="item">
          <ABreadcrumb v-if="item.matched.length > 0">
            <template v-for="(route) in item.matched" :key="route.path">
              <ABreadcrumbItem v-if="route.meta.breadcrumb !== false && route.meta.title">
                <template v-if="route.meta.icon">
                  <SvgIcon v-if="typeof route.meta.icon === 'string'" :icon-name="route.meta.icon as string" />
                  <component :is="route.meta.icon" v-else />
                </template>
