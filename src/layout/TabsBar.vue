...
function closeAllTabs() {
  ...
  for (const tab of tabs) {
    if (tab.path === dashboardRoute.path)
      continue
    nextTick(() => {
      closeTab(tab, true)
    })
...
function handleDrop(e: DragEvent) {
  if (dragIndex.value === undefined || dropIndex.value === undefined || dragIndex.value === dropIndex.value)
    return
...
<template>
  <div class="tabs-bar">
    ...
    <Scrollbar ref="scrollbarDom" height="2rem" direction="horizontal" :speed="5" style="width: 100px; flex: 1;">
      <div class="tabs">
        <div
          v-for="(tab, index) in tabs" ref="tabDoms" :key="tab.path" class="tab"
        ...
        >
          <div
            v-if="tab.meta.keepAlive && props.withDot"
          ...
          <template v-if="props.withIcons && tab.meta.icon">
            <SvgIcon v-if="typeof tab.meta.icon === 'string'" :icon-name="tab.meta.icon as string" />
